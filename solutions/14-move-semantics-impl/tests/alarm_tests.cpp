...
TEST_F(AlarmTest, move_constructor) {
  Alarm a{Alarm::Type::warning, "Move constructor test"};
  Alarm b{std::move(a)};
  ASSERT_EQ(Alarm::Type::invalid, a.type());
  ASSERT_EQ(Alarm::Type::warning, b.type());
  ASSERT_EQ(std::string_view{"invalid"}, std::string_view{a.to_string()});
  ASSERT_EQ(std::string_view{"warning : Move constructor test"}, std::string_view{b.to_string()});
}

TEST_F(AlarmTest, move_assignment) {
  Alarm a{Alarm::Type::caution, "Move assignment test"};
  Alarm b = std::move(a);
  ASSERT_EQ(Alarm::Type::invalid, a.type());
  ASSERT_EQ(Alarm::Type::caution, b.type());
  ASSERT_EQ(std::string_view{"invalid"}, std::string_view{a.to_string()});
  ASSERT_EQ(std::string_view{"caution : Move assignment test"}, std::string_view{b.to_string()});}
